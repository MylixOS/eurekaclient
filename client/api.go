...
func Register(zone, app string, instance *Instance) error {
	...
	result := httpclient.Post(u).Json(info).Send().Status2xx()
	if result.Err != nil {
...
func UnRegister(zone, app, instanceID string) error {
	...
	// status: http.StatusNoContent
	result := httpclient.Delete(u).Send().StatusOk()
	if result.Err != nil {
...
func Refresh(zone string) (*Applications, error) {
	...
	u := zone + "apps"
	err := httpclient.Get(u).Header("Accept", " application/json").Send().StatusOk().Json(res)
	if err != nil {
...
func Heartbeat(zone, app, instanceID string) error {
	...
	result := httpclient.Put(u).Params(params).Send()
	if result.Err != nil {
